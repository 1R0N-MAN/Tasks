...
class OngoingRecyclerAdapter(private val tasks: ArrayList<OngoingTasks>): RecyclerView.Adapter<OngoingRecyclerAdapter.MyViewHolder>() {
        ...
        holder.taskCheckBox.setOnCheckedChangeListener { compoundButton, _ ->
            run {
                if (compoundButton.isChecked){
                    ...
                    val completedTask = CompletedTasks(currentTask.taskName, currentTask.deadline, currentTask.difficulty)

                    dao?.insertCompletedTask(completedTask)
                    dao?.deleteOngoingTask(currentTask)
    ...
    private fun deleteTask(position: Int, dialog: Dialog){
        val dao = db!!.getTasksDao()
        dao.deleteOngoingTask(tasks[position])
